 ...
 *      "....")
 * @see 举例：假设一个SSH2工程中有个名为userLogin()的Action方法，其内部调用了UserService，
 *      UserService内部又调用了UserDAO
 * @see 我们在userLogin()、UserService、UserDAO中均使用了LogUtil.getLogger().info("....")
 *      记录相应的日志信息
...
public class LogUtil {
	 ...
	 * @see <code>LogUtil.getLogger().error("请求被拒绝or请求地址有误,堆栈信息如下", cause)</code>
	 * @see 最后,记得会话开始时,通过
	 *      <code>session.setAttribute("currentLog", LogUtil.getLogger())</code>
	 *      标记日志记录器
	 */
